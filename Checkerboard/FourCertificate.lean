...
theorem sumFiber_le_two {n : ℕ} {s : Finset (Point n)}
  ...
  intro a b c hab hac
  have habv : a.1.1 + a.2.1 = b.1.1 + b.2.1 := congrArg Fin.val hab
  have hacv : a.1.1 + a.2.1 = c.1.1 + c.2.1 := congrArg Fin.val hac
...
private theorem weightedFibers_le_two
    ...
    (∑ i, (fiberCard s f i : ℚ) * weight i) ≤ 2 * ∑ i, weight i := by
  rw [Finset.mul_sum]
...
theorem fourCertificate_bound {n : ℕ} {s : Finset (Point n)}
  ...
  have hs : (∑ p ∈ s, w.sum (sumIndex p)) ≤ 2 * ∑ i, w.sum i := by
    ...
    exact weightedFibers_le_two s sumIndex w.sum hsum (sumFiber_le_two hntil)
  have hd : (∑ p ∈ s, w.difference (differenceIndex p)) ≤ 2 * ∑ i, w.difference i := by
...
theorem card_le_of_fourCertificate {n k : ℕ} {q : ℚ}
  ...
  have hltQ : (s.card : ℚ) < k + 1 := by nlinarith
  have hltN : s.card < k + 1 := by exact_mod_cast hltQ
  omega
